...
#include <game/server/Data/zuowangdao_names.h>
...
int CServer::TrySetClientName(int ClientID, const char *pName, bool Bot)
{
...
void CServer::SetClientName(int ClientID, const char *pName, bool Bot)
{
	...
	if (TrySetClientName(ClientID, aCleanName, Bot))
	{
		// auto rename
		for (int i = 1;; i++)
		{
			...
			if (TrySetClientName(ClientID, aNameTry, Bot) == 0)
				break;
		}
	}
}
...
void CServer::SendServerInfo(const NETADDR *pAddr, int Token, bool Extended, int Offset)
{
	...
	{
		if (ClientCount < SNAP_MAX_CLIENTS)
	...
	if (!Extended)
	{
		if (ClientCount >= SNAP_MAX_CLIENTS)
		{
			if (ClientCount < MaxClients)
				ClientCount = SNAP_MAX_CLIENTS - 1;
			else
				ClientCount = SNAP_MAX_CLIENTS;
		}
		if (MaxClients > SNAP_MAX_CLIENTS) MaxClients = SNAP_MAX_CLIENTS;
	}
	...
	str_format(aBuf, sizeof(aBuf), "%d", MaxClients - g_Config.m_SvSpectatorSlots);
	p.AddString(aBuf, 3); // max players
	str_format(aBuf, sizeof(aBuf), "%d", ClientCount);
	p.AddString(aBuf, 3); // num clients
	str_format(aBuf, sizeof(aBuf), "%d", MaxClients);
	p.AddString(aBuf, 3); // max clients

	if (Extended)
		p.AddInt(Offset);

	int ClientsPerPacket = Extended ? 24 : SNAP_MAX_CLIENTS;
	int Skip = Offset;
...
void CServer::UpdateServerInfo()
{
	for (int i = 0; i < MAX_PLAYERS; ++i)
	{
		if (m_aClients[i].m_State != CClient::STATE_EMPTY)
		{
			const NETADDR *Addr = m_NetServer.ClientAddr(i);
	...
	while (m_NetServer.Recv(&Packet))
	{
		if (Packet.m_ClientID == -1)
		{
			...
			if (!m_Register.RegisterProcessPacket(&Packet))
			{
				...
				{
					SendServerInfo(&Packet.m_Address, ((unsigned char *)Packet.m_pData)[sizeof(SERVERBROWSE_GETINFO)], false);
				}
			}
		}
		else
